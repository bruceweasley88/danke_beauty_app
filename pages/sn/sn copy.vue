<template>
	<view class="page-container">
		...
		<live-pusher id="livePusher" ref="livePusher" class="live-pusher" url="" mode="FHD" :autoplay="true"
			:camera="cameraPosition" :muted="true" :enable-mic="false" @statechange="start"></live-pusher>

		<!-- 扫描动画 -->
		<cover-view v-if="permissionReady" class="scan-line-container">
			<cover-image src="/static/scan_icon/img_light@2x.webp" class="scan-line"
				:style="{ top: scanLineTop + 'rpx' }"></cover-image>
		</cover-view>

		<!-- 扫描结果提示 -->
		<cover-view v-if="scanResult && permissionReady" class="scan-result-overlay">
			<cover-view class="result-text">{{ scanResult }}</cover-view>
		</cover-view>

		<!-- 扫描提示文字 -->
		<cover-view v-if="permissionReady" class="scan-tip">请对准设备二维码，即可自动扫描</cover-view>

		<!-- 底部按钮区域 -->
		<cover-view v-if="permissionReady" class="action-buttons">
